...
import { EcosystemForm, EcosystemFormData } from "./EcosystemForm";
import { deleteEcosystemNode, reorderEcosystemNodes } from "./actions";

export function EcosystemClient({
  nodes,
}: {
  nodes: EcosystemFormData[];
}) {
  const [items, setItems] = useState(nodes);
  const [editing, setEditing] = useState<EcosystemFormData | null | "new">(
    null
  );
  ...
  function handleDrop(targetIndex: number) {
    ...
    setDragIndex(null);
    startTransition(() => {
      void reorderEcosystemNodes(next.map((n) => n.id!));
    });
  }
