...
import { upsertFocusItem } from "./actions";
...
export function FocusForm({
  ...
  initial: FocusFormData | null;
  onClose: () => void;
}) {
  const [state, formAction, isPending] = useActionState(upsertFocusItem, {});
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-surface border border-border rounded-2xl max-w-sm w-full p-6 my-8 space-y-4 relative">
        ...
        <h2 className="font-display font-semibold text-lg text-text-primary">
          {initial?.id ? "Edit Focus Item" : "Tambah Focus Item"}
        </h2>
